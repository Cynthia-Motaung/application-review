...
const mockApplicationData = {
    ...
    creditCheckStatus: "complete" as const,
    incomeDocsStatus: "verified" as const,
    
    documents: [
        { name: "ID Document.pdf", status: "Verified" as const, uploaded: "Oct 8, 2024" }, 
        { name: "Proof of Address.png", status: "Verified" as const, uploaded: "Oct 8, 2024" },
        { name: "Bank Statement.pdf", status: "Under Review" as const, uploaded: "Oct 8, 2024" },
        { name: "Payslip.pdf", status: "Verified" as const, uploaded: "Oct 8, 2024" }
    ],
    timeline: [
...
const fetchData = async (id: number) => {
    return new Promise(resolve => {
        setTimeout(() => {
            ...
            resolve(mockApplicationData);
        }, 500);
    });
};

const postDecision = async (id: number, decision: string, notes: string) => {
    return new Promise(resolve => {
        setTimeout(() => {
...
export default function ApplicationReview() {
    ...
    useEffect(() => {
        const loadData = async () => {
            setIsLoading(true);
            try {
                const fetchedData = await fetchData(applicationId) as ApplicationData;
                setData({
                    ...fetchedData,
    ...
    const handleDecisionMade = useCallback(async (decision: string, notes: string) => {
        ...
        try {
            const response = await postDecision(applicationId, decision, notes) as { success: boolean; newStatus: string };
            if (response.success) {
